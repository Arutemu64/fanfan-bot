...
from bot.ui.strings import buttons, menus
...
announce_mode = Window(
    Const(menus.announce_mode_text),
    Button(text=Const(buttons.next), id="next", on_click=send_next),
    Start(
        text=Const(buttons.show_schedule),
        id="show_schedule",
        state=states.SCHEDULE.MAIN,
        data={"back_to": states.ANNOUNCE_MODE.MAIN},
    ),
    Start(text=Const(buttons.back), id="helper_menu", state=states.HELPER.MAIN),
...
preview_announcement = Window(
    ...
    Const(" "),
    Const("Отправляем? 📣"),
    Button(Const(buttons.send), id="send_button", on_click=send_announce),
    Back(Const(buttons.cancel)),
