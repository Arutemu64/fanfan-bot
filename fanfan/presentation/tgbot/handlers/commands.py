...
async def link_ticket_cmd(message: Message, dialog_manager: DialogManager):
    await dialog_manager.start(states.MAIN.LINK_TICKET)


@router.message(Command(QR_CMD))
async def qr_pass_cmd(message: Message, dialog_manager: DialogManager):
