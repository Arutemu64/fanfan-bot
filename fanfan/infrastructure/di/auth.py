...
from fanfan.infrastructure.auth.providers.raw import RawIdProvider
...
class TelegramAuthProvider(Provider):
    ...
    @provide
    async def get_telegram_id_provider(self, obj: TelegramObject) -> IdProvider:
        if obj.from_user:
            return RawIdProvider(obj.from_user.id)
        return RawIdProvider(None)
