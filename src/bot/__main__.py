...
async def setup_default_settings(engine: AsyncEngine) -> None:
...
async def main() -> None:
    ...
    await setup_default_settings(engine)
    ...
    if conf.bot.mode == "webhook":
        ...
        app.router.add_post(f"{conf.bot.webhook_path}/qr_scanner", proceed_qr_post)

        webhook_requests_handler = SimpleRequestHandler(
            dispatcher=dp, bot=bot, engine=engine
        )
